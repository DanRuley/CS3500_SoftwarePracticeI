...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        {
            //Method for building the dependency list and checking for cycles.  Utilizes a topological sort algorithm for the graph traversal.

            //add current node to visited
            visited.Add(name);

            //foreach node that [name] has an edge pointing towards...
            foreach (String n in GetDirectDependents(name))
            {
                //if n == start, we've returned to a previously visited node, thus the graph contains a cycle.
                if (n.Equals(start))
                {
                    throw new CircularException();
                }

                //otherwise, add n to the list of visited nodes
                else if (!visited.Contains(n))
                {
                    Visit(start, n, visited, changed);
                }
            }
            //Add this node to the beginning of the Linked List
